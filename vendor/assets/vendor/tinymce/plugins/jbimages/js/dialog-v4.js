...
var jbImagesDialog = {
	...
	uploadFinish : function(result) {
		if (result.resultCode == 'failed') {
			...
			tinymce.EditorManager.activeEditor.insertContent('<img src="' + result.filename +'" alt="upload-image" />');
			//var dummy = tinymce.EditorManager.activeEditor.contentDocument.getElementById('dummy');
			//tinymce.EditorManager.activeEditor.selection.select(dummy);
			//tinymce.EditorManager.activeEditor.selection.setContent('<br data-mce-bogus="1">');
			
			//tinymce.EditorManager.activeEditor.selection.setCursorLocation(tinymce.activeEditor.selection.getNode(), 1);
			//tinymce.EditorManager.activeEditor.focus();
			
			//var evt = this.getWin().document.createEvent("KeyboardEvent");
			//(evt.initKeyEvent || evt.initKeyboardEvent)("keypress", true, true, w.window, 0, 0, 0, 0, 27, 0)
			//var canceled = !this.getWin().document.getElementsByClassName('editor-content').dispatchEvent(evt);
			
			//console.log('upload finished', tinymce.activeEditor.execCommand('mceAutoResize', false));
			this.close();
		}
	},
	getWin : function() {
	...
	close : function() {
		...
		function close() {
			...
			tinymce = tinyMCE = t.editor = t.params = t.dom = null; // Cleanup  //t.dom.doc = 
		};
		
		// 이미지 로드되는 시간동안 딜레이하고 resize command 수동으로 실행
		this.getWin().setTimeout(function() { console.log('timeout', tinymce.activeEditor.execCommand('mceAutoResize', false)); close(); }, 150);
